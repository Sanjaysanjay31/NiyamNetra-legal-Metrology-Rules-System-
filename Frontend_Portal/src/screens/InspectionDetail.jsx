...
function getRowChecklistData(scan, row) {
  // 1. Check scan.checklist array (if present from engine payload)
...
export default function InspectionDetail() {
  ...
  const detail = useResource(
    async () => {
      ...
      if (res && res.scans && res.scans.length > 0 && res.scans[0].id) {
        try {
  ...
  const overallVerdict = d?.resultVerdict ?? scan?.overall_result ?? (d?.status === 'submitted' ? 'compliant' : 'not_assessed')
  ...
  const shop = useMemo(() => {
    ...
    return {
      name: d?.store_name || d?.storeName || `Shop #${d?.store_id ?? '—'}`,
      city: d?.area || '—',
      address: d?.store_address || '—',
      license: d?.store_license || '—',
    }
  }, [shops.data, d])
  ...
  const officer = useMemo(() => {
    ...
    if (fromApi) return fromApi
    return {
      full_name: d?.inspectorName || `Officer #${d?.user_id ?? '—'}`,
      employee_id: d?.inspectorId || '—',
    }
  }, [officers.data, d])
