...
**In-memory state multiplies by the worker count.** The login limiter used to hold counters in a dict (permitting `4 × limit` with four workers); it is now DB-backed (`login_attempts`), so every worker enforces the same count. The TTL/divergence note still holds for the remaining process-local caches in §2.6. Both are acceptable if the multiplication is *known*; the failure is asserting "10 requests per minute" while shipping forty.
