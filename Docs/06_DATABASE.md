...
**`in_scope` is always a boolean set at create — there is no NULL gate.** `routers/inspections.py` computes `in_scope = transaction_type in _RETAIL_TYPES` (`{"retail_sale"}`) and stores it directly, so every inspection row has TRUE or FALSE from the start and submission is never refused on a NULL scope. `transaction_type` drives it: `retail_sale` is IN-SCOPE (Chapter II applies); `wholesale`, `institutional`, `industrial`, `packed_in_presence`, `export` and `other` are out of scope at the inspection level, with per-package applicability still recorded by CHK03. An earlier revision of this paragraph (and of `03` §CHK03 and `04` §Phase 1) listed `packed_in_presence` as in-scope; it is made up in the purchaser's presence and therefore exempt from the pre-packaged-commodity rules — `_RETAIL_TYPES = {"retail_sale"}` in the router and `OUT_OF_SCOPE_TRANSACTIONS` in the rules engine both say so, and the engine is the operative definition (05 §1.1: one rules engine, one answer).
